...
<?php $component->withAttributes([]); ?>
    <div class="mx-auto w-full max-w-6xl space-y-8 px-2 sm:px-4 lg:px-6">
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!$userPackage): ?>
            <section class="rounded-2xl border border-slate-200 bg-white px-6 py-12 shadow-sm text-center md:px-8">
                ...
                <a href="<?php echo e(route('investment-plans')); ?>" class="mt-6 inline-flex items-center justify-center rounded-full bg-cyan-600 px-6 py-3 text-sm font-semibold uppercase tracking-[0.12em] text-white shadow-sm transition-all hover:bg-cyan-700 active:scale-95">
                    Browse Plans
                </a>
            </section>
        <?php else: ?>
                    ...
                    <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
                        ...
                        <div class="space-y-8">
                            ...
                            <div>
                                ...
                                <p class="text-3xl font-bold text-slate-900"><?php echo e($userPackage->package->name ?? 'N/A'); ?></p>
                            </div>
                                    ...
                                    <ul class="space-y-2">
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $userPackage->package->facilities; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $facility): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <li class="flex items-start gap-3">
                                                ...
                                                <span class="text-sm text-slate-700"><?php echo e($facility); ?></span>
                                            </li>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p class="text-lg font-semibold text-slate-900"><?php echo e($userPackage->starts_at ? $userPackage->starts_at->format('M d, Y') : 'Pending'); ?></p>
                                </div>
                                <div>
                                    <p class="text-xs font-semibold uppercase tracking-[0.08em] text-slate-600 mb-2">End Date</p>
                                    <p class="text-lg font-semibold text-slate-900"><?php echo e($userPackage->ends_at ? $userPackage->ends_at->format('M d, Y') : 'N/A'); ?></p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-8">
                            ...
                            <div>
                                ...
                                <span class="inline-flex items-center rounded-full px-4 py-2 text-sm font-semibold uppercase tracking-[0.08em]
                                    <?php if($userPackage->status === 'active'): ?> bg-emerald-100 text-emerald-800
                                    <?php elseif($userPackage->status === 'expired'): ?> bg-rose-100 text-rose-800
                            ...
                            <div>
                                ...
                                <p class="text-3xl font-bold text-cyan-700"><?php echo e($userPackage->equity ? '$' . number_format($userPackage->equity, 2) : 'N/A'); ?></p>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="space-y-2 text-sm">
                                    <p class="text-slate-700"><span class="font-semibold text-slate-900">Broker:</span> <?php echo e($userPackage->broker_name ?? 'N/A'); ?></p>
                                    <p class="text-slate-700"><span class="font-semibold text-slate-900">Trading ID:</span> <?php echo e($userPackage->trading_id ?? 'N/A'); ?></p>
                                    <p class="text-slate-700"><span class="font-semibold text-slate-900">Server:</span> <?php echo e($userPackage->trading_server ?? 'N/A'); ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            <tr class="border-b border-slate-200 transition-colors hover:bg-slate-50">
                                <td class="px-6 py-4">
                                    <div class="font-semibold text-slate-900"><?php echo e($userPackage->package->name ?? '—'); ?></div>
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!empty($userPackage->package?->facilities) && is_array($userPackage->package->facilities)): ?>
                                        <div class="mt-1 text-xs text-slate-600">
                                            <?php echo e(implode(' • ', $userPackage->package->facilities)); ?>

                                        </div>
                                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                </td>
                                <td class="px-6 py-4 text-slate-600"><?php echo e($userPackage->starts_at ? $userPackage->starts_at->format('M d, Y') : '—'); ?></td>
                                <td class="px-6 py-4 text-slate-600"><?php echo e($userPackage->ends_at ? $userPackage->ends_at->format('M d, Y') : '—'); ?></td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.08em]
                                        <?php if($userPackage->status === 'active'): ?> bg-emerald-100 text-emerald-800
                                        <?php elseif($userPackage->status === 'expired'): ?> bg-rose-100 text-rose-800
                                        <?php else: ?> bg-amber-100 text-amber-800 <?php endif; ?>">
                                        <?php echo e(ucfirst($userPackage->status)); ?>

                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-3">
                                        <a href="<?php echo e(route('investment-plans')); ?>" class="inline-flex items-center rounded-full bg-cyan-600 px-4 py-2 text-xs font-semibold uppercase tracking-[0.12em] text-white transition-all hover:bg-cyan-700 active:scale-95">
                                            Change Plan
                                        </a>

                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($userPackage->status !== 'expired'): ?>
                                            <form method="POST" action="<?php echo e(route('my-plans.cancel', $userPackage)); ?>" class="inline" onsubmit="return confirm('Are you sure you want to cancel this plan?');">
                                                <?php echo csrf_field(); ?>
                                        ...
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($userPackage->status === 'expired'): ?>
                                            <form method="POST" action="<?php echo e(route('my-plans.renew', $userPackage)); ?>" class="inline">
                                                <?php echo csrf_field(); ?>
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\traderscap\resources\views/customer/my-plans.blade.php ENDPATH**/ ?>