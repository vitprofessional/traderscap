    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 z-40 w-64 bg-[#0f172a] text-slate-50 shadow-xl flex flex-col transition-transform duration-300 -translate-x-full md:translate-x-0">
        ...
        <div class="px-5 py-5 flex items-center justify-between border-b border-slate-700/60">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-sm font-bold text-white leading-tight">TradersCap</h2>
        ...
        <nav class="flex-1 px-3 py-4 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-700">
            ...
            @foreach($navSections as $section => $items)
                <p class="mt-5 mb-1.5 px-3 text-[10px] font-bold tracking-widest text-slate-500 uppercase">{{ $section }}</p>
                <ul class="space-y-0.5">
                    @foreach($items as $it)
                        ...
                        @endphp
                        <li>
                            <a href="{{ $locked ? '#' : $routeUrl }}" class="group flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-150 {{ $isActive ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-100' }} {{ $locked ? 'opacity-60 cursor-not-allowed' : '' }}">
                                ...
                                <span class="text-sm font-medium flex-1 leading-none">{{ $it['label'] }}</span>
                                @if($locked)
        ...
        <div class="mx-3 mb-4 mt-2 rounded-xl bg-gradient-to-br from-cyan-600 to-cyan-800 p-4">
            ...
            <a href="{{ route('partners') }}" class="mt-1 flex items-center justify-center gap-2 w-full rounded-lg bg-white/20 hover:bg-white/30 transition-colors px-3 py-2 text-xs font-semibold text-white">
                Learn More
    ...
    <div class="flex flex-col min-h-screen md:pl-64">
        ...
        @php
            ...
            if ($topUser && !empty($topUser->avatar)) {
                $topAvatarSrc = str_starts_with($topUser->avatar, 'http') ? $topUser->avatar : asset('storage/app/public/' . $topUser->avatar);
            }
        ...
        <header class="sticky top-0 z-20 bg-white border-b border-slate-200/80" style="box-shadow: 0 1px 3px 0 rgba(0,0,0,.06), 0 1px 2px -1px rgba(0,0,0,.06);">
            <div class="flex items-center justify-between h-16 px-4 sm:px-6">
                ...
                <div class="flex items-center gap-3 min-w-0">
                    ...
                    <div class="hidden sm:flex items-center gap-2 min-w-0">
                        ...
                        <div class="flex items-center gap-1.5 text-sm">
                            <span class="text-slate-400 font-medium">TradersCap</span>
                    ...
                    <span class="sm:hidden text-sm font-bold text-slate-900 truncate">{{ $currentPage }}</span>
                </div>
                ...
                <div class="flex items-center gap-1 sm:gap-2">
                        ...
                        <div id="notifications-menu" class="absolute right-0 mt-2 w-80 bg-white rounded-2xl border border-slate-200 shadow-2xl opacity-0 invisible transition-all duration-200 z-50" style="pointer-events: none;">
                            ...
                            <div class="max-h-72 overflow-y-auto divide-y divide-slate-50">
                                ...
                                <a href="#" class="flex items-start gap-3 px-4 py-3.5 hover:bg-slate-50 transition-colors group">
                                    <div class="w-9 h-9 rounded-full bg-{{ $notif['color'] }}-100 flex items-center justify-center flex-shrink-0 mt-0.5">
                                        @if($notif['icon']==='dollar')
                                            <svg class="w-4 h-4 text-{{ $notif['color'] }}-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                        @elseif($notif['icon']==='check')
                                            <svg class="w-4 h-4 text-{{ $notif['color'] }}-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                        @else
                                            <svg class="w-4 h-4 text-{{ $notif['color'] }}-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4v.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                        @endif
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-medium text-slate-800 group-hover:text-slate-900 leading-snug">{{ $notif['title'] }}</p>
                            ...
                            <div class="border-t border-slate-100 px-4 py-3 flex items-center justify-between">
                                <a href="{{ route('complaints') }}" class="text-xs font-semibold text-cyan-600 hover:text-cyan-700 transition-colors">View all notifications →</a>
                    ...
                    {{-- Support --}}
                    <a href="{{ route('complaints') }}" class="w-9 h-9 flex items-center justify-center rounded-lg text-slate-500 hover:text-slate-900 hover:bg-slate-100 transition-colors" title="Support">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                        ...
                        <button id="user-avatar-btn" class="flex items-center gap-2.5 rounded-xl pl-1 pr-2.5 py-1 hover:bg-slate-100 transition-colors cursor-pointer group">
                            ...
                            <div class="relative flex-shrink-0">
                                <div class="w-8 h-8 rounded-full overflow-hidden ring-2 ring-slate-200 group-hover:ring-cyan-400 transition-all">
                                    <img src="{{ $topAvatarSrc ?? $topAvatarFallback }}" alt="avatar" class="w-full h-full object-cover">
                                </div>
                                <span class="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-emerald-500 ring-2 ring-white"></span>
                            </div>
                            {{-- Name + role --}}
                            <div class="hidden sm:block text-left">
                                <p class="text-sm font-semibold text-slate-900 leading-tight">{{ $topUser->name ?? 'Guest' }}</p>
                        ...
                        <div id="user-menu" class="absolute right-0 mt-2 w-60 bg-white rounded-2xl border border-slate-200 shadow-2xl opacity-0 invisible transition-all duration-200 z-50" style="pointer-events: none;">
                            ...
                            <div class="px-4 py-4 border-b border-slate-100">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <p class="text-sm font-bold text-slate-900 truncate">{{ $topUser->name ?? 'Guest' }}</p>
                                        <p class="text-xs text-slate-500 truncate">{{ $topUser->email ?? '' }}</p>
                            ...
                            <div class="py-2 px-1.5">
                                <a href="{{ route('profile') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-slate-700 hover:bg-slate-50 transition-colors">
                                ...
                                <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-slate-700 hover:bg-slate-50 transition-colors">
                                    <span class="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center flex-shrink-0">
                                ...
                                <a href="{{ route('complaints') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-slate-700 hover:bg-slate-50 transition-colors">
                                    <span class="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center flex-shrink-0">
