<section class="rounded-2xl border border-slate-200 bg-white shadow-sm">
    ...
    <div class="h-96 space-y-3 overflow-y-auto px-6 py-5 md:px-8" wire:poll.3s id="chat-messages">
        @foreach($messages as $m)
            <div class="flex gap-3 {{ $m->is_admin ? 'flex-row' : 'flex-row-reverse' }}">
                <!-- Avatar -->
                <div class="flex-shrink-0">
                    <div class="flex h-9 w-9 items-center justify-center rounded-full {{ $m->is_admin ? 'bg-cyan-100' : 'bg-slate-200' }}">
                        <span class="text-xs font-semibold {{ $m->is_admin ? 'text-cyan-700' : 'text-slate-700' }}">
                ...
                <div class="flex-1">
                    <div class="flex items-baseline gap-2">
                        @if($m->user)
                            <span class="text-xs font-semibold text-slate-900">{{ $m->user->name }}</span>
                        @endif
                        <span class="text-xs text-slate-500">{{ $m->created_at->diffForHumans() }}</span>
                    </div>
                    <div class="mt-1 rounded-lg px-4 py-3 {{ $m->is_admin ? 'bg-cyan-50' : 'bg-slate-100' }}">
                        <p class="text-sm leading-relaxed text-slate-900">{!! nl2br(e($m->message)) !!}</p>
                        @if($m->attachment)
                            <div class="mt-3 flex items-center text-xs font-semibold text-cyan-600 hover:text-cyan-700">
                                ...
                                <a href="{{ asset('storage/app/public/'.$m->attachment) }}" target="_blank" class="ml-1 hover:underline">View attachment</a>
                            </div>
                        @endif
                    </div>
                </div>
